...
class Correlation {
	...
	public static double[] pVector(MultiLog ml) {
		int numTrials = ml.logs.size();
		int[][] correct = new int[numTrials][];
		double[] p = new double[numTrials];
		...
		for(int i=0; i<numTrials;i++){
			...
			p[i] = total/(double)correct[i].length;
		}
		return p;
	}
	...
	private static boolean isCorrect(MultiLog ml, int i, int t) {
		...
		return s1[1].equals(s2)
				&& ml.logs.get(i).tests.get(t).results.get(0).rank != ml.logs.get(i).tests.get(t).results.get(1).rank;
	}
	...
	public static void exportCSV(double[][] mat, ArrayList<String> methods, String name) {
		...
		if (mat.length == methods.size() && mat[0].length == methods.size()) {
			...
			System.err.println("Number of Method Names (" + methods.size()
					+ ") does not match the size of the provided array's columns" + "and rows" + "(" + mat.length + ", "
					+ mat[0].length + ")");
		}
	}
	...
	public static void singletonExportCSV(double[] mat, ArrayList<String> methods, String name, String method) {
		...
		if (mat.length == methods.size()) {
			...
			System.err.println("Number of Method Names (" + methods.size()
					+ ") does not match the size of the provided array's columns" + "(" + mat.length + ")");
		}
	}
